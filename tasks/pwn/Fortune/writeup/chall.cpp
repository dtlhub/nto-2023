...
int main() {
    ...
    std::cout << "Welcome to the Fortune telling tent!" << std::endl;
    std::cout  << "Lay out the cards and see your destiny:" << std::endl;
    ...
    for (std::string line; std::getline(ss, line, ',');) {
        ...
        if (card_value == Cards.end()) {
            std::cout << "The fortune is not on your side...." << std::endl;
            return 1;
        }
        shellcode.push_back(card_value -> second);
    }
    if (shellcode.size() > 0x1000) {
        std::cout  << "The cards are telling me that you will lose..." << std::endl;
        return 1;
    }
